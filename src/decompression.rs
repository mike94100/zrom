...
/// Decompress Zip (first file only)
pub fn unpack_zip(input: &Path, output: &Path) -> Result<Stats, ZromError> {
    let input_bytes = input.metadata()?.len();
    let mut archive = zip::ZipArchive::new(File::open(input)?).map_err(|e| ZromError::Io(e.to_string()))?;
    let mut file = archive.by_index(0).map_err(|e| ZromError::Io(e.to_string()))?;
    io::copy(&mut file, &mut File::create(output)?)?;
    let output_bytes = output.metadata()?.len();
    Ok(Stats { input_bytes, output_bytes })
}
